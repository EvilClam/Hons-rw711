...
When a particle collides with the boundary it changes its direction by $180^{\circ}$, note by moving a particle to the opposite side is the same as rotating by $180^{\circ}$. It then propagates in that direction.
\begin{figure}[H]
...
\paragraph{Rule 1}(2-particle head-on collisions):\\This collision rule was added in FHP-I. When two opposite sites are occupied and the rest of the sites are empty,  then a collision has happened. Now are two configurations to choose from. The choice between the two configurations are done randomly with a probability of 0.5. The rule can be defined as: If \[(s_{i} = s_{i + 3})\wedge(s_{i} = 1)\wedge(s_{j} = 0)\wedge(j \neq i)\wedge(j \neq (i + 3)) \mbox{ for } i,j \in \{0, 1, 2, 3, 4, 5\}\] then with p = 0.5 choose \[ (s_{i - 1} := s_{i}), (s_{i + 2} := s_{i + 3})\]or\[(s_{i + 1} := s_{i}), (s_{i + 4} := s_{i + 3}).\]
\begin{figure}[H]
 ...
 \coordinate (Origin) at (0 + \x,0 + \y);
  ...
  \end{minipage}
  \label{figure:Hcollision_FHP2}
\end{figure}
\paragraph{Rule 2}(symmetric 3-particle collisions):\\
This collision rule was added in FHP-I. When three particles collide at a $120^{\circ}$ their directions are rotated by a $180^{\circ}$. The rule can be defined as: If \[(s_{i} = s_{i + 2} = s_{i + 4})\wedge(s_{i + 1} = s_{i + 3} = s_{i + 5})\wedge(s_{i} \neq s_{i + 1})\mbox{ for } i \in \{0, 1, 2, 3, 4, 5\}\] then if $s_{i} = 0$\[ (s_{i} := s_{i + 1}), (s_{i + 2} := s_{i + 3}), (s_{i + 4} := s_{i + 5})\]
else\[ (s_{i + 1} := s_{i}), (s_{i + 3} := s_{i + 2}), (s_{i + 5} := s_{i + 4}).\]
\begin{figure}[H]
